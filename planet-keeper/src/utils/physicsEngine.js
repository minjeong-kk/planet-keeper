 ...
 * ------------------------------------------------------------------
 *
 * - TOA 하향단파복사속 기준 상수 S (KIM 실측)  → 아래 SOLAR_CONSTANT
 * - CO₂ 기준 농도 (기상청 관측소 3곳 2024년 실측 평균) → 아래 CO2_BASELINE_PPM
 *   두 값은 physics-merge.py가 재수집할 때마다 선언부를 자동으로 덮어쓴다 -
 *   여기(설명)에 숫자를 또 적으면 갱신에서 빠져 조용히 낡으므로 적지 않는다.
...
// ── 과학적 기준 상수 (PLAN.md) ──────────────────────────────────
// TOA 하향단파복사. KIM 전지구 필드 평균 실측값(physics_reference.csv의 dswrtoa)을
// 소수 둘째 자리로 반올림해서 쓴다 - 이 줄의 숫자는 physics-merge.py가 재수집
// 결과로 자동으로 덮어쓰므로 손으로 고치지 않는다.
//
...
export const SOLAR_CONSTANT = 296.4 // TOA 하향단파복사속 기준 (KIM 실측)
// ΔE 단위(W/m²)로 표현된 임계값을 가진 다른 모듈들도 이 배율을 써서 환산한다.
export const ENERGY_SCALE = SOLAR_CONSTANT / 100 // 계획서 기준 스케일(S=100) 대비 배율
// 기상청 관측소 3곳(울릉도·독도 / 안면도 / 고산)의 2024년 월별 실측 36개를 평균한
// 값(physics_reference.csv의 co2 - 관측소별 원값은 co2_data.csv에 있다). 계획서는
// 432로 두고 시작했는데 그건 어느 관측소 값과도 맞지 않아서 실측 평균으로 교체했다.
// 아래 선언부 숫자도 physics-merge.py가 자동으로 덮어쓴다.
//
