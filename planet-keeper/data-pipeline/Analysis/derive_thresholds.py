...
관측 t2m의 '폭'만 쓰고 '중심'은 계획서 기준값을 쓴다.
  ...
  - observed_kim_dataset.csv의 t2m 평균은 지구 평균(288.15K)과 완전히 같지 않다.
    좌표는 전지구에서 균등 무작위로 뽑지만(observed-kim.py), KMA API 조회 기간이
    최근 약 180일이라 계절이 2~8월로 한정된다 - LIMITATIONS.md 2번에 기록된 편향이다.
    ...
    return {
        ...
        "derivation": {
            ...
            "note": (
                ...
                "관측 표본이 KIM 조회 제한(약 180일)으로 계절이 한정돼 있어(LIMITATIONS.md "
                "2번) 관측 평균이 아니라 계획서 기준값을 중심으로 삼는다."
            ),
        },
    }
