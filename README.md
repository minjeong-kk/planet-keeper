...
| 기준 행성 알베도 | 0.28 | (0.229) | ❌ 계획서 유지 | 아래 ③ |
| 온실효과 기준 `g` | 0.30 | (0.412) | ❌ 계획서 유지 | 아래 ③ |
...
**③ 알베도·온실효과 — 실측 쪽에 미해결 편향이 있어 채택하지 않았습니다.**

KIM 전지구 필드 평균에서 무차원 비율을 뽑으면 다음과 같습니다.

```
행성 알베도  = 1 − ulwrtoa / dswrtoa = 1 − 229.67 / 297.88 = 0.229
유효 방출률  = ulwrtoa / σT⁴         = 229.67 / 390.92     = 0.588  →  g = 0.412
```

그런데 분모인 `dswrtoa` 실측 전지구 평균 **297.88 W/m²는 이론값 340.3 W/m²(= 태양상수
1361 ÷ 4)보다 12.5% 낮습니다.** 원인을 특정하지 못했고, 이 편향은 위 비율에 그대로
전파됩니다.

| | 행성 알베도 | 온실효과 `g` |
|---|---|---|
| KIM 실측 그대로 | 0.229 | 0.412 |
| 12.5% 편향 보정 시 | — | 0.329 |
| **계획서 (채택)** | **0.280** | **0.300** |
| 실제 지구 | ≈0.300 | ≈0.386 |

알베도는 계획서 값(0.28)이 실제 지구(0.30)에 더 가깝고, 온실효과는 편향 보정 여부에
따라 0.33~0.41로 값이 정해지지 않습니다. **실측을 쓰면 오히려 실제 지구에서 멀어지거나
값이 하나로 결정되지 않으므로 채택하지 않았습니다.** 편향 원인이 규명되면 재검토 대상입니다.
