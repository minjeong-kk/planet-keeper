...
cp ../Models/climate_rf.onnx ../../public/models/climate_rf.onnx
# 게임(프론트)에 반영
```

> `train_rf.py` / `export_onnx.py` / `evaluate.py`를 수정했으면
> 위 순서대로 다시 실행해야 모델, ONNX, 평가 결과, 프론트 반영본이 서로 어긋나지 않습니다.
...
| **run_physics_engine.mjs** | `generate_dataset.py`가 `src/utils/physicsEngine.js`(JS)를 그대로 호출하기 위한 Node 브릿지. |
